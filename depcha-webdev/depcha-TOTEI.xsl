...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="@xml:id[not(ancestor::*:facsimile)] | @ref[not(ancestor::*:unitDef)] | @commodity| @unit | @unitRef" priority="-1">
        <xsl:attribute name="{name()}" select="lower-case(.)"> </xsl:attribute>
    </xsl:template>
    ...
    <xsl:template match="@xml:id[parent::*[tokenize(@ana, ' ') = 'bk:entry']]"> </xsl:template>
